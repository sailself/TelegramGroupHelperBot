...
- **bot/handlers/commands.py**
  - Implements `/tldr`, `/factcheck`, `/img`, `/vid`, `/paintme` `/portraitme`, `/profileme`, `/qq` (Gemini quick question: default Gemini model, no model selection), `/start`, `/help`, `/support`, plus media-group caching.
...
- Thinking level: `GEMINI_THINKING_LEVEL` defaults to `high`; commands can override when needed. URL Context is opt-in (`use_url_context=False` by default)—enable only when link context is desired.
